...
public class SucaiLiuyanController {
    ...
    */
    ...
    public R update(@RequestBody SucaiLiuyanEntity sucaiLiuyan, HttpServletRequest request){
...
//            sucaiLiuyan.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        //根据字段查询是否有相同数据
        Wrapper<SucaiLiuyanEntity> queryWrapper = new EntityWrapper<SucaiLiuyanEntity>()
            .eq("id",0)
            ;

        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        SucaiLiuyanEntity sucaiLiuyanEntity = sucaiLiuyanService.selectOne(queryWrapper);
        sucaiLiuyan.setUpdateTime(new Date());
        if(sucaiLiuyanEntity==null){
...
//                            sucaiLiuyanEntity.setSucaiId(Integer.valueOf(data.get(0)));   //素材 要改的
...
//                            sucaiLiuyanEntity.setUpdateTime(new Date(data.get(0)));          //回复时间 要改的
